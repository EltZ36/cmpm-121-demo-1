...
createUpgradeButtons(availableItems);

//makes the upgrade buttons form the upgrade list
function createUpgradeButtons(upgradeList: Item[]) {
...
function getUpgrade(upgradeList: Item[], index: number) {
...
function canUpgrade(upgradeList: Item[]) {
  for (let i = 0; i < upgradeList.length; i++) {
